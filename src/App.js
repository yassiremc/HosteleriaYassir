...
function App() {
        ...
        const restaurantData = restaurantDocs.map((docItem) => {
          ...
          const street =
            ...
            (Array.isArray(locationField)
              ? locationField.join(', ')
              : (locationField && typeof locationField === 'object')
                ? 'No disponible'
                : String(locationField || 'No disponible'));
  ...
  const handleDeleteRestaurant = async () => {
    if (!selectedRestaurant) return;
    const confirmed = window.confirm(`Vols eliminar el restaurant "${selectedRestaurant.name}"?`);
    if (!confirmed) return;
    try {
      await fetch(`${FIRESTORE_BASE_URL}/Restaurant/${encodeURIComponent(selectedRestaurant.id)}`, { method: 'DELETE' });
    } catch (error) {
      // ignore remote delete failure and continue local cleanup
    }
                  ...
                  <article key={restaurant.id} className="restaurant-card restaurant-card-clickable">
                    ...
                    >
                      ...
                      <div className="restaurant-card-body">
                        ...
                        <div className="restaurant-alumni-row">
                          <div className="restaurant-alumni-avatars">
                            {students.filter((student) => student.restaurantId === restaurant.id).slice(0, 3).map((student) => (
                              <img
                                key={`avatar-${restaurant.id}-${student.id}`}
              ...
              <div className="restaurant-profile-header">
                ...
                <div>
                  ...
                  <div className="restaurant-action-row">
                    <button type="button" className="pill-button" onClick={handleEditRestaurant}>Editar</button>
